...
Eigen::Vector3d ClosestPointInTetrahedron(const vMF<3>& vmf_A, const
  ...
  for (uint32_t i=0; i<4; ++i)
    mus[i] = tetrahedron.GetVertexQuaternion(i).toRotationMatrix()*vmf_B.GetMu();
  ...
  for (auto tri : combinations.Get()) {
    ...
    if (qr.rank() == 3) {
      Eigen::Vector3d a = qr.solve(vmf_A.GetMu());
      if ((a.array() > 0.).all()) {
        return vmf_A.GetMu();
      }
    }
...
Eigen::Vector3d FurthestPointInTetrahedron(const vMF<3>& vmf_A, const
  ...
  return ClosestPointInTetrahedron(vmf_A_neg, vmf_B, tetrahedron);
}
}
